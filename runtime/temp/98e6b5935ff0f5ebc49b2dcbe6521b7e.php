...
<?php endif; switch($hisiTabType): case "1": ?>
        ...
        <li>
            <img src="<?php echo htmlentities($vo['thumb']); ?>" width="150" height="150">
            <dl>
                <dd>名称：<span class="mcolor"><?php echo htmlentities($vo['title']); ?></span></dd>
                <dd>作者：<span class="mcolor2"><?php echo htmlentities($vo['author']); ?></span></dd>
                <dd>版本：<span class="mcolor2"><?php echo htmlentities($vo['version']); ?></span></dd>
                <dd>时间：<span class="mcolor2"><?php echo htmlentities($vo['time']); ?></span></dd>
                <dt><div class="layui-btn-group">
                        <?php if(($vo['name'] == $formData['theme'])): ?><a class="layui-btn layui-btn-primary layui-btn-xs layui-btn-disabled">默认</a><?php if(($vo['sql'])): ?><a data-href="<?php echo url('exeSql?app_name='.$formData['name'].'&theme='.$vo['name']); ?>" class="layui-btn layui-btn-xs layui-btn-warm hisi-ajax" confirm="此操作可能会清空当前模块下的所有数据<br>请提前做好备份工作。切记！切记！">导入SQL</a><?php endif; else: ?><a data-href="<?php echo url('setDefaultTheme?id='.$formData['id'].'&theme='.$vo['name']); ?>" class="layui-btn layui-btn-xs layui-btn-normal hisi-ajax">设为默认</a><?php if(($vo['sql'])): ?><a data-href="<?php echo url('exeSql?app_name='.$formData['name'].'&theme='.$vo['name']); ?>" class="layui-btn layui-btn-xs layui-btn-warm hisi-ajax" confirm="此操作可能会清空当前模块下的所有数据<br>请提前做好备份工作。切记！切记！">导入SQL</a><?php endif; ?><?php endif; if((isset($vo['identifier']) && !empty($vo['identifier']))): ?><a href="<?php echo url('upgrade/lists?app_name='.$formData['name'].'&app_type=theme&app_version='.$vo['version'].'&identifier='.$vo['identifier']); ?>" class="layui-btn layui-btn-normal layui-btn-xs">更新</a><?php endif; ?><a data-href="<?php echo url('delTheme?id='.$formData['id'].'&theme='.$vo['name']); ?>" class="layui-btn layui-btn-danger layui-btn-xs hisi-ajax" title="删除之后将无法恢复！您确定要执行删除吗？">删除</a></div></dt>
            </dl>
        </li>
        <?php endforeach; endif; else: echo "" ;endif; ?>
